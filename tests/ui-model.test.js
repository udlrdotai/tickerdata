...
function fixture() {
  ...
  return {
    ...
    vocabulary: {
      ...
      tags: [label('tag-old', '旧标签'), label('tag-target', '目标标签'), label('tag-unused', '未使用标签')],
    },
  };
}
...
test('search includes ticker, names, provider aliases, historical symbols and MIC', () => {
  ...
  assert.equal(matchesRecord(record, { tag: 'tag-old', review: 'reviewed' }), true);
  assert.equal(matchesRecord(record, { tag: 'tag-target' }), true);
  assert.equal(matchesRecord(record, { tag: 'tag-unused' }), false);
  record.classification.tag_ids = [];
  assert.equal(matchesRecord(record, {}), true);
  assert.equal(matchesRecord(record, { tag: 'tag-old' }), false);
});
...
test('explicit review accepts zero tags and preserves independent fields and evidence', () => {
  ...
  assert.deepEqual({ ...reviewed, review: before.review }, before);
...
test('renaming a vocabulary label preserves IDs and flags references', () => {
  ...
  vocabulary.tags[0].name_zh = '新显示名称';
  const current = applyVocabulary(initial, vocabulary);
  assert.equal(current.vocabulary.tags[0].id, 'tag-old');
  assert.deepEqual(current.instruments[0].classification.tag_ids, ['tag-old', 'tag-target']);
...
test('referenced deletion is refused; unreferenced deletion is allowed', () => {
  ...
  vocabulary.tags = vocabulary.tags.filter((item) => item.id !== 'tag-old');
  assert.throws(() => applyVocabulary(initial, vocabulary), /不能删除被引用/);
  vocabulary = clone(initial.vocabulary);
  vocabulary.tags = vocabulary.tags.filter((item) => item.id !== 'tag-unused');
  assert.equal(applyVocabulary(initial, vocabulary).vocabulary.tags.length, 2);
...
test('tag merging is immutable, atomic, preserves aliases and exports every changed file', () => {
  ...
  targetRecord.classification.tag_ids = ['tag-target'];
  initial.instruments.push(targetRecord);
  const current = mergeVocabulary(initial, 'tags', 'tag-old', 'tag-target');
  assert.equal(initial.vocabulary.tags.length, 3);
  assert.equal(current.vocabulary.tags.length, 2);
  ...
  assert.deepEqual(current.vocabulary.tags[0].aliases, ['旧标签']);
...
test('tag merging deduplicates targets and rejects invalid IDs', () => {
  ...
  assert.throws(() => mergeVocabulary(initial, 'themes', 'a', 'b'), /只支持标签/);
});
...
test('atomic merge cascades related review dependencies and passes actual dataset validation', () => {
  ...
  dependent.classification.tag_ids = ['tag-other'];
  ...
  const merged = mergeVocabulary(initial, 'tags', 'tag-old', 'tag-target');
  ...
  const vocabulary = clone(initial.vocabulary);
  vocabulary.tags[0].description = '更新标签定义';
