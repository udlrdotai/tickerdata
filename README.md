...
该命令自动构建并启动临时本地测试服务，覆盖编辑、审核、新增 ETF、标签合并及直接提交 PR，不需要更改正式源记录。Playwright 与浏览器不进入静态网页运行时。
...
完整字段含义见 [数据模型 4.0.0](docs/data-model.md)。行业明细见 [FinanceDatabase 分类](docs/industry-classification.md)。许可边界见 [许可说明](docs/licensing.md)。
...
5. 在“内存草稿 / 完整变更清单”中确认受影响文件，填写分支名、提交信息、PR 标题和描述，由 Worker 一次性创建新分支与 PR。多文件标签合并必须把词表和所有引用放进**同一个 PR**。
6. CI 校验通过后由人审阅合入。重新构建站点；正式消费文件仅包含 `review.status=reviewed` 的记录。

**审核不是一个自动通过的按钮。** 审核人需要核对身份、MIC、已填写的行业、标签和 ETF 属性，以及行业 / ETF 来源，填写 `reviewer`、UTC `reviewed_at`。标签不是审核必填项，也不维护来源。已审核记录修改后应变为 `needs_review`，或经过明确复核并填写比旧记录更新的审核时间。PR 校验也会检查这一点。

### 从 1.0.0 / 2.0.0 显式迁移

移除标签来源后使用严格的 `4.0.0` 协议，旧文件不能直接用于当前维护页面，也不能只修改 `schema_version`。仓库管理员可使用原始 `1.0.0`、`2.0.0` 或 `3.0.0` 单证券、完整词表或维护包进行一次性协议迁移预览：
...
若本地仍是旧版数据，迁移会一并升级完整维护数据集，避免同一目录混用协议。旧词表若会移除被现有证券引用的分类，会被拒绝。涉及多个依赖文件时使用完整维护包。

迁移哈希同时绑定当前源数据和本次输入内容，任一变化均需重新预览。已经是 4.0.0 的输入不能再次迁移。纯结构转换可保留已有审核状态、署名和时间；实际修改内容仍受审核降级规则约束，旧文件也不能自动授予一次新审核。迁移不 commit、不发布、不改历史 release 或建议文件；日常维护仍只通过网页创建 GitHub PR。

旧建议分别使用保留的 `schemas/v1/`、`schemas/v2/`、`schemas/v3/` 结构检查，保留原始版本和基准记录哈希，不适用于 4.0.0 源记录；需要重新生成或人工重新评估，不能通过修改哈希让旧建议自动生效。

### 标签维护

所有自定义分类方向统一为多选标签，使用稳定 ID。新增词条时应填写与词义相关的英文小写 slug（如 `satellite-communication`）；保存后 ID 不可修改，显示名重命名不会改变 ID。标签名称及别名去空白、不区分大小写后不能冲突。
...
网页维护只支持通过 GitHub PR 持久保存。纯静态服务器或未配置 GitHub App 的 GitHub Pages 只能预览，不能保存变更。部署 Worker 后，应先在页面顶部登录 GitHub，再开始编辑；OAuth 会重新加载页面，已有内存草稿不会跨登录跳转保留。随后在“内存草稿 / 完整变更清单”中点击“直接提交 GitHub PR”：
...
当前协议为 `4.0.0`，记录不含 `primary_theme_id` 和 `classification.source_ids`，词表不含 `themes`；新的词表产物为 `vocabulary.json`，不再输出 `themes.json`。Python 查询结果返回 `tags` 标签对象数组，不再返回 `primary_theme`。下游需同步更新文件名和读取代码。读取历史快照时使用对应版本消费者，或先显式迁移旧维护源数据并重新发布。旧 release 的字节、哈希和历史建议基准不修改。
...
已覆盖 schema、ID / 别名冲突、分类引用、股票 / ETF 规则、审核迁移、PR 提交、版本产物、建议隔离及 Python 查询 / 缓存行为。校验器能阻止结构错误，**不能证明 ticker 身份或分类判断正确**，这仍需人工审阅。
