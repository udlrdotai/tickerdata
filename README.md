...
取消主主题后使用严格的 `3.0.0` 协议，旧文件不能直接用于当前维护页面，也不能只修改 `schema_version`。仓库管理员可使用原始 `1.0.0` 或 `2.0.0` 单证券、完整词表或维护包进行一次性协议迁移预览：
...
迁移哈希同时绑定当前源数据和本次输入内容，任一变化均需重新预览。已经是 3.0.0 的输入不能再次迁移。纯结构转换可保留已有审核状态、署名和时间；实际修改内容仍受审核降级规则约束，旧文件也不能自动授予一次新审核。迁移不 commit、不发布、不改历史 release 或建议文件；日常维护仍只通过网页创建 GitHub PR。
