...
`classification` 只包含 `tag_ids`。`tag_ids` 是去重后的标签 ID 数组，可选零个、一个或多个，已审核证券也可以没有标签。非空标签必须引用词表中的条目。不再单独维护主主题，不要求用某个标签作为唯一分组，也不维护标签的字段级来源。
...
| `fields` | 来源覆盖字段，如 `/industry`、`/etf`；也支持身份、名称、发行人、上市状态、备注。 |

`industry.source_ids` 和 `etf.source_ids` 各自指向覆盖该字段的来源。行业或 ETF 属性非空时必须有对应依据；保留的来源引用仍需有效且覆盖对应字段。标签不关联来源。身份和名称来源用 `sources.fields` 直接指明。
...
`review.reviewed_at` 是最近一次人工审核时间，待复核时可保留上次时间作参考。旧已审核内容修改后，要么降为待复核，要么明确重新审核并更新时间。这个状态机由网页编辑逻辑与 PR 基准比较共同约束。

所有源记录、词表、建议、发布文件都有 `schema_version`。移除标签来源是严格协议变更，当前源数据和发布产物使用 4.0.0；新词表产物为 `vocabulary.json`，不再输出 `themes.json`。消费者遇到其他版本拒绝读取，不能只改版本号冒充迁移完成。历史快照保持原字节和哈希，使用对应旧版本消费者读取，或显式迁移旧维护源数据后生成新的发布版本；不要直接修改 release 附件。
