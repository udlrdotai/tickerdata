...
`industry.source_ids`、`classification.source_ids`、`etf.source_ids` 各自指向覆盖该字段的来源，不能用“整条来自 Yahoo”掩盖交易主题实际来自人工。分类非空时必须有对应依据。身份和名称来源用 `sources.fields` 直接指明。
...
`review.status` 为 `pending`、`reviewed`、`needs_review`。只有 `reviewed` 进入正式产物，且需要英文名称、MIC、一个主主题、分类依据、审核人和 UTC 审核时间。行业允许未知。不能用满足 schema 代替事实核查。
