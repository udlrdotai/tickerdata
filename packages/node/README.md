...
Offline, versioned ticker metadata lookup for Node.js. The package contains a
fixed snapshot of reviewed metadata and never accesses the network.

```js
import { lookup } from 'tickerdata';

const instrument = lookup('BRK-B', {
  mic: 'XNYS',
  provider: 'yahoo',
...
```js
const { lookup } = require('tickerdata');
```

`lookup` does not rewrite punctuation or guess between ambiguous listings.
Unknown and ambiguous queries throw `UnknownSymbolError` and
